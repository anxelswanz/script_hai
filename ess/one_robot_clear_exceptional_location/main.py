...
def unload_all_containers():
    url = f"http://10.201.160.113:9000/ess-api/model/queryModelByCode?modelCode={robot_id}"
    logger.info("检查背篓是否有箱")
    response = requests.get(url, timeout=10)
    res_data = response.json()
    robot_data = res_data.get("data", {}).get("robot", {})
    trayLoadingContainerCode = res_data.get("trayLoadingContainerCode", {})
    if not trayLoadingContainerCode:
        print("机器人没有任何箱子")
        return
    container_list = [str(cid) for cid in trayLoadingContainerCode.values() if cid]
    for container in container_list:
        min_distance, closest_put_location = distance_calculation(locations)
        createAction(robot_id, "unload", closest_put_location, container)
